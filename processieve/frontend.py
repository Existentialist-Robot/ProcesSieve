...
def show():
    ...
    with ui.column().classes("w-full p-4"):
        ...
        folder_url_input = ui.input(
            label="Google Drive Shared Folder URL", placeholder="Enter folder URL"
        ).classes("w-full")
        set_folder_button = ui.button(
            "Set Folder URL", on_click=lambda: on_set_folder_url(folder_url_input.value)
        )
        ...
        create_doc_button = ui.button(
            "Create Document",
            on_click=lambda: state.create_document(
        ...
        create_sheet_button = ui.button(
            "Create Spreadsheet",
            on_click=lambda: state.create_spreadsheet(
...
ui.run_with(
    ...
    mount_path="/",
    storage_secret=(
        config.get("main", "secret", fallback=None)
